// src/components/hardware_status/HardwareStatusTable.jsx
import React, { useState, useMemo, useRef, useEffect } from 'react';
import {
  X, ShieldCheck, Edit, Trash2, Search, ArrowDown, ArrowUp, ArrowLeft, ArrowRight
} from 'lucide-react';

export default function HardwareStatusTable({ hardwareStatuses, onEdit, onDelete }) {
  ...
  const searchFields = useMemo(() => {
    ...
    hardwareStatuses.forEach(status => {
      if (status.name) fields.add(status.name);
    });
  ...
  const getSortValue = (hardwareStatus, key) => {
    switch (key) {
      case 'name':
        return hardwareStatus.name || '';
      case 'description':
        return hardwareStatus.description || '';
      default:
        return '';
    }
  };

  const sortedHardwareStatuses = useMemo(() => {
    const filtered = hardwareStatuses.filter(status => {
      if (activeTags.length === 0) {
        return true;
      }
      return activeTags.every(tag => {
        const lowerCaseTag = tag.toLowerCase();
        const searchables = [status.name, status.description].filter(Boolean).map(val => String(val).toLowerCase());
    ...
    return filtered;
  }, [hardwareStatuses, activeTags, sortConfig]);

  const indexOfLastHardwareStatus = currentPage * rowsPerPage;
  const indexOfFirstHardwareStatus = indexOfLastHardwareStatus - rowsPerPage;
  const currentHardwareStatuses = sortedHardwareStatuses.slice(indexOfFirstHardwareStatus, indexOfLastHardwareStatus);
  const totalPages = Math.ceil(sortedHardwareStatuses.length / rowsPerPage);
  const totalResults = sortedHardwareStatuses.length;
  ...
  if (!hardwareStatuses || hardwareStatuses.length === 0) {
    return (
      <div className="flex justify-center items-center h-40 bg-gray-100 rounded-lg shadow-inner">
        <p className="text-gray-500 font-medium text-lg">No hardware statuses found.</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-lg shadow-md">
        ...
        >
          <div className="relative">
            <input
              type="text"
              ref={searchInputRef}
              placeholder="Search hardware statuses..."
              value={searchQuery}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentHardwareStatuses.length > 0 ? (
              currentHardwareStatuses.map((status) => (
              ...
              <tr>
                <td colSpan="3" className="px-6 py-4 text-center text-gray-500">
                  No hardware statuses found matching your search.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="px-6 py-4 flex items-center justify-between border-t border-gray-200">
        ...
        <div className="text-sm text-gray-600">
          Showing {indexOfFirstHardwareStatus + 1} to {Math.min(indexOfLastHardwareStatus, totalResults)} of {totalResults} results
        </div>
        <div className="flex items-center space-x-2">
          <button
