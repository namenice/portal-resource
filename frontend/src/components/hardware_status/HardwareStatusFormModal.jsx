// src/components/hardware_status/HardwareStatusFormModal.jsx
import React, { useState, useEffect, useCallback } from 'react';
import { ShieldCheck, X } from 'lucide-react';

export default function HardwareStatusFormModal({ isOpen, onClose, onSubmit, hardwareStatusToEdit }) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (hardwareStatusToEdit) {
        setName(hardwareStatusToEdit.name || '');
        setDescription(hardwareStatusToEdit.description || '');
      } else {
        setName('');
        setDescription('');
      }
  ...
  }, [hardwareStatusToEdit, isOpen]);

  const validate = () => {
    const newErrors = {};
    if (!name.trim()) {
      newErrors.name = 'Hardware status name is required.';
    }
    setErrors(newErrors);
  ...
  }, [name, description, onSubmit, onClose, hardwareStatusToEdit]);
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-white">
                  {hardwareStatusToEdit ? 'Edit Hardware Status' : 'Add New Hardware Status'}
                </h3>
                <p className="mt-1 text-blue-100 text-sm">
                  {hardwareStatusToEdit ? 'Update hardware status information' : 'Create a new hardware status'}
                </p>
              </div>
              <button
                onClick={onClose}
              ...
              <div className="group">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Enter hardware status name (e.g., Active)"
                    disabled={isLoading}
                  />
                  <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                    <ShieldCheck className={`h-5 w-5 transition-colors duration-200 ${errors.name ? 'text-red-400' : 'text-gray-400'}`} />
                  </div>
                </div>
                {errors.name && (
