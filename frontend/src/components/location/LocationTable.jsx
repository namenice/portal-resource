// src/components/location/LocationTable.jsx
import React, { useState, useMemo, useRef, useEffect } from 'react';
import {
  X, MapPin, Edit, Trash2, Search, ArrowDown, ArrowUp, ArrowLeft, ArrowRight
} from 'lucide-react';
...
export default function LocationTable({ locations, onEdit, onDelete }) {
  ...
  const searchFields = useMemo(() => {
    ...
    locations.forEach(location => {
      if (location.site_name) fields.add(location.site_name);
      if (location.room) fields.add(location.room);
      if (location.rack) fields.add(location.rack);
    });
  ...
  const getSortValue = (location, key) => {
    switch (key) {
      case 'site_name':
        return location.site_name || '';
      case 'room':
  ...
  const sortedLocations = useMemo(() => {
    ...
    return filtered;
  }, [locations, activeTags, sortConfig]);

  const indexOfLastLocation = currentPage * rowsPerPage;
  ...
  const totalResults = sortedLocations.length;
  ...
  if (!locations || locations.length === 0) {
    return (
      <div className="flex justify-center items-center h-40 bg-gray-100 rounded-lg shadow-inner">
        <p className="text-gray-500 font-medium text-lg">No locations found.</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-lg shadow-md">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th
                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none"
                onClick={() => requestSort('rack')}
              >
                Rack
                {renderSortArrow('rack')}
              </th>
                ...
                <tr key={location.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="text-sm font-medium text-gray-900">{location.site_name}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{location.room}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{location.rack}</td>
      ...
      <div className="px-6 py-4 flex items-center justify-between border-t border-gray-200">
        ...
        <div className="text-sm text-gray-600">
          Showing {indexOfFirstLocation + 1} to {Math.min(indexOfLastLocation, totalResults)} of {totalResults} results
        </div>
        <div className="flex items-center space-x-2">
          <button
