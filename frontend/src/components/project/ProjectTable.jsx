...
import {
  X, Briefcase, Edit, Trash2, Search, ArrowDown, ArrowUp, ArrowLeft, ArrowRight
} from 'lucide-react';
...
export default function ProjectTable({ projects, onEdit, onDelete }) {
  ...
  const searchFields = useMemo(() => {
    ...
    projects.forEach(project => {
      if (project.name) fields.add(project.name);
      if (project.owner) fields.add(project.owner);
    });
  ...
  const sortedProjects = useMemo(() => {
    const filtered = projects.filter(project => {
      ...
      return activeTags.every(tag => {
        ...
        const searchables = [project.name, project.owner, project.description].filter(Boolean).map(val => String(val).toLowerCase());
    ...
    return filtered;
  }, [projects, activeTags, sortConfig]);
    ...
    <div className="bg-white rounded-lg shadow-md">
      ...
      <div className="px-6 py-4 flex items-center justify-between border-t border-gray-200">
        ...
        <div className="text-sm text-gray-600">
          Showing {indexOfFirstProject + 1} to {Math.min(indexOfLastProject, totalResults)} of {totalResults} results
        </div>
        <div className="flex items-center space-x-2">
          <button
