// src/components/hardware_model/HardwareModelTable.jsx
import React, { useState, useMemo, useRef, useEffect } from 'react';
import {
  X, Server, Edit, Trash2, Search, ArrowDown, ArrowUp, ArrowLeft, ArrowRight
} from 'lucide-react';

export default function HardwareModelTable({ hardwareModels, onEdit, onDelete }) {
  ...
  const searchFields = useMemo(() => {
    ...
    hardwareModels.forEach(model => {
      if (model.brand) fields.add(model.brand);
      if (model.model) fields.add(model.model);
    });
  ...
  const getSortValue = (hardwareModel, key) => {
    switch (key) {
      case 'brand':
        return hardwareModel.brand || '';
      case 'model':
        return hardwareModel.model || '';
      case 'description':
        return hardwareModel.description || '';
      default:
        return '';
    }
  };

  const sortedHardwareModels = useMemo(() => {
    const filtered = hardwareModels.filter(model => {
      if (activeTags.length === 0) {
        return true;
      }
      return activeTags.every(tag => {
        const lowerCaseTag = tag.toLowerCase();
        const searchables = [model.brand, model.model, model.description].filter(Boolean).map(val => String(val).toLowerCase());
    ...
    return filtered;
  }, [hardwareModels, activeTags, sortConfig]);

  const indexOfLastHardwareModel = currentPage * rowsPerPage;
  const indexOfFirstHardwareModel = indexOfLastHardwareModel - rowsPerPage;
  const currentHardwareModels = sortedHardwareModels.slice(indexOfFirstHardwareModel, indexOfLastHardwareModel);
  const totalPages = Math.ceil(sortedHardwareModels.length / rowsPerPage);
  const totalResults = sortedHardwareModels.length;
  ...
  if (!hardwareModels || hardwareModels.length === 0) {
    return (
      <div className="flex justify-center items-center h-40 bg-gray-100 rounded-lg shadow-inner">
        <p className="text-gray-500 font-medium text-lg">No hardware models found.</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-lg shadow-md">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentHardwareModels.length > 0 ? (
              currentHardwareModels.map((model) => (
              ...
              <tr>
                <td colSpan="4" className="px-6 py-4 text-center text-gray-500">
                  No hardware models found matching your search.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="px-6 py-4 flex items-center justify-between border-t border-gray-200">
        ...
        <div className="text-sm text-gray-600">
          Showing {indexOfFirstHardwareModel + 1} to {Math.min(indexOfLastHardwareModel, totalResults)} of {totalResults} results
        </div>
        <div className="flex items-center space-x-2">
          <button
