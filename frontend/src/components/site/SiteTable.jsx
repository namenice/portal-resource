// src/components/site/SiteTable.jsx
...
export default function SiteTable({ sites, onEdit, onDelete }) {
  ...
  const searchFields = useMemo(() => {
    ...
    sites.forEach(site => {
      if (site.name) fields.add(site.name);
    });
  ...
  const totalResults = sortedSites.length;
  ...
  if (!sites || sites.length === 0) {
    return (
      <div className="flex justify-center items-center h-40 bg-gray-100 rounded-lg shadow-inner">
        <p className="text-gray-500 font-medium text-lg">No sites found.</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-lg shadow-md">
      ...
      <div className="px-6 py-4 flex items-center justify-between border-t border-gray-200">
        ...
        <div className="text-sm text-gray-600">
          Showing {indexOfFirstSite + 1} to {Math.min(indexOfLastSite, totalResults)} of {totalResults} results
        </div>
        <div className="flex items-center space-x-2">
          <button
