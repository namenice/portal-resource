...
import {
  X, User, Edit, Trash2, Search, ArrowDown, ArrowUp, ArrowLeft, ArrowRight
} from 'lucide-react';
...
export default function UserTable({ users, onEdit, onDelete }) {
  ...
  const searchFields = useMemo(() => {
    ...
    users.forEach(user => {
      if (user.username) fields.add(user.username);
    });
  ...
  const sortedUsers = useMemo(() => {
    const filtered = users.filter(user => {
      ...
      return activeTags.every(tag => {
        ...
        const searchables = [user.username].filter(Boolean).map(val => String(val).toLowerCase());
    ...
    <div className="bg-white rounded-lg shadow-md">
      ...
      <div className="px-6 py-4 flex items-center justify-between border-t border-gray-200">
        ...
        <div className="text-sm text-gray-600">
          Showing {indexOfFirstUser + 1} to {Math.min(indexOfLastUser, totalResults)} of {totalResults} results
        </div>
        <div className="flex items-center space-x-2">
          <button
