// src/components/hardware_type/HardwareTypeTable.jsx
import React, { useState, useMemo, useRef, useEffect } from 'react';
import {
  X, Server, Edit, Trash2, Search, ArrowDown, ArrowUp, ArrowLeft, ArrowRight
} from 'lucide-react';

export default function HardwareTypeTable({ hardwareTypes, onEdit, onDelete }) {
  ...
  const searchFields = useMemo(() => {
    ...
    hardwareTypes.forEach(type => {
      if (type.name) fields.add(type.name);
    });
  ...
  const getSortValue = (hardwareType, key) => {
    switch (key) {
      case 'name':
        return hardwareType.name || '';
      case 'description':
        return hardwareType.description || '';
      default:
        return '';
    }
  };

  const sortedHardwareTypes = useMemo(() => {
    const filtered = hardwareTypes.filter(type => {
      if (activeTags.length === 0) {
        return true;
      }
      return activeTags.every(tag => {
        const lowerCaseTag = tag.toLowerCase();
        const searchables = [type.name, type.description].filter(Boolean).map(val => String(val).toLowerCase());
    ...
    return filtered;
  }, [hardwareTypes, activeTags, sortConfig]);

  const indexOfLastHardwareType = currentPage * rowsPerPage;
  const indexOfFirstHardwareType = indexOfLastHardwareType - rowsPerPage;
  const currentHardwareTypes = sortedHardwareTypes.slice(indexOfFirstHardwareType, indexOfLastHardwareType);
  const totalPages = Math.ceil(sortedHardwareTypes.length / rowsPerPage);
  const totalResults = sortedHardwareTypes.length;
  ...
  if (!hardwareTypes || hardwareTypes.length === 0) {
    return (
      <div className="flex justify-center items-center h-40 bg-gray-100 rounded-lg shadow-inner">
        <p className="text-gray-500 font-medium text-lg">No hardware types found.</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-lg shadow-md">
        ...
        >
          <div className="relative">
            <input
              type="text"
              ref={searchInputRef}
              placeholder="Search hardware types..."
              value={searchQuery}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentHardwareTypes.length > 0 ? (
              currentHardwareTypes.map((type) => (
              ...
              <tr>
                <td colSpan="3" className="px-6 py-4 text-center text-gray-500">
                  No hardware types found matching your search.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="px-6 py-4 flex items-center justify-between border-t border-gray-200">
        ...
        <div className="text-sm text-gray-600">
          Showing {indexOfFirstHardwareType + 1} to {Math.min(indexOfLastHardwareType, totalResults)} of {totalResults} results
        </div>
        <div className="flex items-center space-x-2">
          <button
