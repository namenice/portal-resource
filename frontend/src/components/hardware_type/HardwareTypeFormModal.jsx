// src/components/hardware_type/HardwareTypeFormModal.jsx
import React, { useState, useEffect, useCallback } from 'react';
import { Server, X } from 'lucide-react';

export default function HardwareTypeFormModal({ isOpen, onClose, onSubmit, hardwareTypeToEdit }) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (hardwareTypeToEdit) {
        setName(hardwareTypeToEdit.name || '');
        setDescription(hardwareTypeToEdit.description || '');
      } else {
        setName('');
        setDescription('');
      }
  ...
  }, [hardwareTypeToEdit, isOpen]);

  const validate = () => {
    const newErrors = {};
    if (!name.trim()) {
      newErrors.name = 'Hardware type name is required.';
    }
    setErrors(newErrors);
  ...
  }, [name, description, onSubmit, onClose, hardwareTypeToEdit]);
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-white">
                  {hardwareTypeToEdit ? 'Edit Hardware Type' : 'Add New Hardware Type'}
                </h3>
                <p className="mt-1 text-blue-100 text-sm">
                  {hardwareTypeToEdit ? 'Update hardware type information' : 'Create a new hardware type'}
                </p>
              </div>
              <button
                onClick={onClose}
              ...
              <div className="group">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Enter hardware type name (e.g., Server)"
                    disabled={isLoading}
                  />
                  <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                    <Server className={`h-5 w-5 transition-colors duration-200 ${errors.name ? 'text-red-400' : 'text-gray-400'}`} />
                  </div>
                </div>
                {errors.name && (
